...
Prettier plugins are regular JavaScript modules with the following six exports or default export with the following properties:
...
Visitors give each plugin a chance to inspect, modify, and even replace the generated AST. Additionally, each plugin may define one or more visitors per AST in case there's need to restrict specific mutations to a specific parser.

The key must match the `astFormat` procued by a parser. The value contains an object with a `afterParse` function. All other properties (`parser`, `parentParser`, etc.) are optional.
...
This function allows any interested plugins to modify the AST generated by another plugin. Here you can add, mutate, replace, or remove AST nodes. If it is not behavior is undefined. If no return value is specified the AST is assumed to have been modified in-place. The AST modified or returned by the function **MUST** be compatible with the printer for that AST — if not the behavior is undefined.

This can be used to hook into the parsing process without overriding or replacing built-in plugins. For example, this can be used to modify HTML tags or attributes, JSX expressions, JS doc blocks, etc…

```ts
function afterParse(
  ast: AST,
  options: object,
): AST | Promise<AST | void> | void;
```

The `afterParse` function is passed the following parameters:

- **`ast`**: An object, that represents the AST returned by a plugins `parse` function. If the given AST is mutable you may modify this structure in place, otherwise you may return a new object representing the updated AST.
- **`options`**: A persistent object, which contains global options and which a plugin may mutate to store contextual data.

Here’s a simplified example of a version of `afterParse` that changes all `span`s to `div`s in an HTML document.

```js
import { visitAll, RecursiveVisitor } from "angular-html-parser";

function afterParse(ast, options) {
  visitAll(
...
A plugin may restrict what parser a specific visitor is run against. For example, Angular, LWC, and Vue are all represented as HTML and, as such, use the same AST format. However, you may want to only run a given visitor when processing angular code.

```ts
let parser?: string
```

#### (optional) `parentParser`

Since visitors are also run when parsing embedded documents, this option lets you restrict a visitor to the "parent" language when processing them. For example, a markdown file can have multiple code blocks each containing a different language. And further, in a language such as Angular or Vue, some attributes are represented as JS or TS expressions. In these situations the `parser` for JS or TS might be `babel` but the `parentParser` could be `markdown`, `angular`, or `vue`.

```ts
let parentParser?: string
```
